...
/// <summary>
/// Texas has no state individual income tax (prohibited by Tex. Const. art. VIII, § 24,
/// as amended by Proposition 4 in 2019). Withholding is always zero; the adapter returns
/// a descriptive result with no schema fields.
/// </summary>
public class TexasWithholdingCalculatorTest
{
    ...
    [Fact]
    public void State_ReturnsTexas()
    {
        var calc = CreateCalc();
    ...
    {
        // Texas has no income tax, so no state-specific input fields are needed.
        var calc = CreateCalc();
    ...
    {
        // Texas levies no income tax on wages, so withholding is always $0.
        var calc = CreateCalc();
    ...
    {
        // Pre-tax deductions have no effect because there is no TX income tax to begin with.
        var calc = CreateCalc();
    ...
    public void Explanation_NoIncomeTax_SingleStep()
    {
        ...
        Assert.Contains("TX", result.WithholdingReference);
    }
}
