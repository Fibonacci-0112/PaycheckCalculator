...
/// <summary>
/// Nevada has no state individual income tax (prohibited by Nev. Const. art. 10).
/// Withholding is always zero; the adapter returns a descriptive result with no schema fields.
/// </summary>
public class NevadaWithholdingCalculatorTest
{
    ...
    [Fact]
    public void State_ReturnsNevada()
    {
        var calc = CreateCalc();
        Assert.Equal(UsState.NV, calc.State);
    }
    ...
    {
        // Nevada has no income tax, so no state-specific input fields are needed.
        var calc = CreateCalc();
    ...
    {
        // Nevada levies no income tax on wages, so withholding is always $0.
        var calc = CreateCalc();
    ...
    {
        // Pre-tax deductions have no effect because there is no NV income tax to begin with.
        var calc = CreateCalc();
    ...
    public void Explanation_NoIncomeTax_SingleStep()
    {
        ...
        Assert.Contains("NV", result.WithholdingReference);
    }
}
