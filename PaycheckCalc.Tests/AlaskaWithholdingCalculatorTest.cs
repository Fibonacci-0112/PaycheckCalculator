...
/// <summary>
/// Alaska has no state individual income tax (it has never enacted one).
/// Withholding is always zero; the adapter returns a descriptive result with no schema fields.
/// </summary>
public class AlaskaWithholdingCalculatorTest
{
    ...
    [Fact]
    public void State_ReturnsAlaska()
    {
        var calc = CreateCalc();
        Assert.Equal(UsState.AK, calc.State);
    }
    ...
    {
        // Alaska has no income tax, so no state-specific input fields are needed.
        var calc = CreateCalc();
    ...
    {
        // Alaska levies no income tax on wages, so withholding is always $0.
        var calc = CreateCalc();
    ...
    {
        // Pre-tax deductions have no effect because there is no AK income tax to begin with.
        var calc = CreateCalc();
    ...
    public void Explanation_NoIncomeTax_SingleStep()
    {
        ...
        Assert.Contains("AK", result.WithholdingReference);
    }
}
