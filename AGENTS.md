...
`PaycheckCalculator.App` targets `net10.0-android`, `net10.0-ios`, `net10.0-maccatalyst`, and `net10.0-windows`; Apple targets are included only on macOS and the Windows target only on Windows. CI (`.github/workflows/dotnet.yml`) explicitly restores, builds, and tests `PaycheckCalculator.Tests` on Linux, transitively building Core, Shared, API, and Blazor through its project references. MAUI is not built by that workflow; `codeql.yml` runs CodeQL separately.
...
`Pay/SelfEmploymentCalculator.cs` is a separate (annual, non-projected) pipeline for self-employed / 1099 contractors: from annual net earnings (Schedule C net profit) it computes the federal **self-employment tax** — 15.3% (12.4% Social Security to the $184,500 wage base + 2.9% Medicare, both halves) plus 0.9% Additional Medicare over $200k — on 92.35% of earnings, estimates **state income tax** by running the full earnings through the ordinary `StateCalculatorRegistry` engine at an annual frequency (no U.S. state has a separate self-employment tax; state disability/leave levies are not applied), and builds a **quarterly estimated-payment schedule** (Form 1040-ES) into a `SelfEmploymentResult` with a `SelfEmploymentInput`. It does **not** model federal income tax. It is wired in `AddPaycheckCalculatorCore` and surfaced in **both** front-ends via the same calculation-mode toggle.

`Pay/HourlySalaryCalculator.cs` is a standalone rate-conversion pipeline — no taxes, deductions, or W-4 involved. Given either an hourly rate or an annual salary plus hours/week and weeks/year, it converts to the other direction and reports weekly/biweekly/semimonthly/monthly equivalents from the unrounded annual figure. It is wired in `AddPaycheckCalculatorCore` and surfaced in **both** front-ends via the same calculation-mode toggle. Because a rate conversion has no taxes, net pay, or annual projection, the UIs render it through a dedicated card (MAUI `HourlySalaryCardModel`) rather than the paycheck result cards, and the CSV/PDF/print exports stay disabled for it. Both front-ends pre-validate with `PaycheckInputValidator.ValidateHourlySalary` so the calculator's `ArgumentOutOfRangeException` guards are never the user-facing error path.
