...
/// domain model directly so the web front-end carries no MAUI dependency.
/// </remarks>
public static class PaycheckPdfRenderer
{
    private static readonly CultureInfo Usd = CultureInfo.GetCultureInfo("en-US");

    // Colors mirror the Results panel palette.
    ...
    public static byte[] RenderComparison(IReadOnlyList<ComparisonRow> comparison, string nameA, string nameB)
    {
        ...
        layout.Subtitle($"Generated {DateTime.Now.ToString("MMMM d, yyyy", Usd)}  |  Tax tables");
