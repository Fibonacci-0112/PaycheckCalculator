# AGENTS.md - PaycheckCalc.Api

Scope: this file applies to everything under `PaycheckCalc.Api/`.

## Role of this project

`PaycheckCalc.Api` is the optional sync server. It provides ASP.NET Core Identity email/password accounts and minimal API endpoints for syncing saved paychecks, budgets, transactions, recurring bills, and savings goals. It persists sync state through EF Core and PostgreSQL in normal operation, with tests using a lightweight test configuration.
...
- Add/update `PaycheckCalc.Tests` integration tests for endpoint behavior, auth requirements, merge persistence, tombstones, and budget sync changes.
...
dotnet test PaycheckCalc.Tests --filter "FullyQualifiedName~SyncApi"
```
