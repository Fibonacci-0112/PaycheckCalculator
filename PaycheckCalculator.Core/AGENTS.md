# AGENTS.md - PaycheckCalculator.Core

Scope: this file applies to everything under `PaycheckCalculator.Core/`.

## Role of this project

`PaycheckCalculator.Core` is the UI-agnostic calculation engine for paycheck, tax, gross-up, annual projection, budgeting, recurring bill, savings goal, and report logic. Keep it usable from MAUI, Blazor, the API, and tests without taking dependencies on any of those outer layers.
...
- Add or update xUnit tests in `PaycheckCalculator.Tests` for every calculation change.
