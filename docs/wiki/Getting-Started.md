...
CI (`.github/workflows/dotnet.yml`) explicitly restores, builds, and tests `PaycheckCalculator.Tests` on Linux with the pinned .NET 10 SDK. The test project's references build Core, Shared, API, and Blazor transitively; MAUI is not built by this workflow. CodeQL runs separately.
